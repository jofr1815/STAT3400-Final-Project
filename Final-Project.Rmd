...
For my final project, I will be doing an analysis of Non-Voter data, from FiveThirtyEight's non-voter data set. I will first perform an analysis of the basic variables in order to draw preliminary insight as which influence someones likelihood to vote. I will then split the model into two sets, a training set (80%) and a testing set (20%), and create a preliminary prediction model. Then the full data set will be examined, starting with AOV tests to determine which variables are significant. A larger prediction model will be fitted on every variable, and reduced models will be created through VIF backwards elimination. Finally these models will all be compared, and the best model of them will be chosen.

### Background Information

In the last election I did not receive a ballot before election day, despite having an accurate up to date voter registration and address. During the process of finding a time and place to vote, I wondered whether it was worth it or not, and if my vote actually made a difference. Being a student and having the time I was able to vote, though experiences such as my own are extremely common. Thousands if not millions of U.S. citizens have similar experiences every year, and along with many other factors result in an extremely low turnout of eligible voters. In any given presidential election, the U.S. experiences a voter turnout of roughly 55% of the voting population voting and 60% of voting age citizens voting (Pew Research). This figure places the U.S. among the worst voter turnout rates of developed countries. When this project was assigned I went looking for data sets covering a wide variety of topics, and I stumbled across FiverThirtyEight's non-voter data set. After exploring the code book and general information, I was determined to use it for my final project. 
...
```
To obtain all VIFs below 5, variables Q17_1, Q9_3, Q8_1, Q4_1, Q30, Q8_8, Q8_7, Q3_2, Q4_3, Q14, and Q2_4 were removed. The second reduced model saw a decrease in accuracy among the always and never/rarely categories, and a slight increase of accuracy among the sporadic category. Though overall accuracy rate also did not change much. The ANOVA test confirms this, showing that the removed variables are statistically significant with a p value of 0.002. 
...
This project has shown that their are an absolutely massive number of factors that are associated with an individuals voting preferences. Age, education level, income, party affiliation, trust in government, and recent life events (such as losing a job or being diagnosed with COVID19) were all associated with a change in the distribution of voter category. 

In the future, I would like to perform examinations and create graphics for all of the predictors, in addition to performing ANOVA and pair-wise comparisons to determine which are significant and the most influential. These results would be used to eliminate any non-significant predictors, and gain more insight to the psychology of voters. I would also like to learn and apply a few different model selection methods such as LASSO, AIC, or some sort of factor analysis to narrow down which variables are the most important, and build a more accurate model. 